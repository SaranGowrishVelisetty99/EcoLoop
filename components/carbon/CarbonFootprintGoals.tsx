...
import { Target, Flag, TrendingDown, X, CheckCircle, Clock, AlertCircle } from 'lucide-react';
          ...
          return (
            <div key={goal.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs ${status.bg} ${status.color}`}>
                      <StatusIcon className="inline mr-1" size={10} /> {status.label}
                    </span>
                  </div>
                ...
                <Button variant="ghost" size="sm" className="text-red-400 hover:text-red-300" onClick={() => handleDeleteGoal(goal.id)}>
                  <X size={16} />
                </Button>
              </div>
